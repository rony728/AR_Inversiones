...
export function filterProducts(
  ...
  const resolved =
    typeof filters === "string"
      ? { ...emptyProductFilters, search: filters }
      : filters;
  ...
  return rows.filter((row) => {
    ...
    const matchesCategory =
      !resolved.categoryId || row.categoria_id === resolved.categoryId;
    const matchesStatus =
      resolved.status === "all" ||
      (resolved.status === "active" ? row.activo : !row.activo);
    const matchesStock =
      resolved.stock === "all" ||
      (resolved.stock === "with" ? quantity > 0 : quantity === 0);
    return (
      matchesSearch &&
      matchesCategory &&
      matchesStatus &&
...
export function uniqueNumericValues(
  rows: ProductRow[],
  field: "cantidad_disponible" | "costo_promedio" | "precio_venta",
) {
  return [
    ...new Set(rows.map((row) => Number(row[field])).filter(Number.isFinite)),
...
export function sortProducts(
  ...
  return [...rows].sort((left, right) => {
    ...
    if (sort === "categoria")
      return (
        String(left.categoria ?? "").localeCompare(
          String(right.categoria ?? ""),
          "es",
          { sensitivity: "base" },
        ) * factor
      );
    if (sort === "estado")
      return (Number(left.activo) - Number(right.activo)) * factor;
    const field =
      sort === "cantidad"
        ? "cantidad_disponible"
        : sort === "costo"
...
export function productProfit(
  product: Pick<ProductRow, "costo_promedio" | "precio_venta">,
) {
