...
import { ArrowDownAZ, ArrowUpAZ, Boxes, Camera, Eye, FolderTree, ImagePlus, Pencil, Plus, Search, Upload, X } from 'lucide-react';
...
import { emptyProductFilters, filterProducts, productProfit, sortProducts, type ProductFilters, type ProductRow, type ProductSort, type SortDirection } from '../lib/product-catalog';
...
export function ProductsPage() {
  ...
  const sortableHeading = (field: ProductSort, label: string) => <button type="button" className="sort-button" onClick={() => changeSort(field)}>{label}{sort === field && sortIcon}</button>;
  ...
  return <>
    ...
    <section className="panel product-filter-panel"><div className="product-filter-top"><div className="search product-search"><Search size={17} /><input value={filters.search} onChange={(event) => setFilter('search', event.target.value)} placeholder="Buscar por código, producto o categoría…" /></div><div className="product-stock-quick"><button type="button" className={filters.stock === 'with' ? 'active' : ''} onClick={() => setFilter('stock', filters.stock === 'with' ? 'all' : 'with')}>Con existencia</button><button type="button" className={filters.stock === 'without' ? 'active' : ''} onClick={() => setFilter('stock', filters.stock === 'without' ? 'all' : 'without')}>Agotados</button></div><button type="button" className="secondary compact" onClick={() => setFilters(emptyProductFilters)}>Limpiar filtros</button></div><div className="product-filter-grid"><label>Categoría<select value={filters.categoryId} onChange={(event) => setFilter('categoryId', event.target.value)}><option value="">Todas</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.nombre}</option>)}</select></label><label>Estado<select value={filters.status} onChange={(event) => setFilter('status', event.target.value as ProductFilters['status'])}><option value="all">Todos</option><option value="active">Activos</option><option value="inactive">Inactivos</option></select></label><RangeFilter label="Existencia" minimum={filters.stockMin} maximum={filters.stockMax} onMinimum={(value) => setFilter('stockMin', value)} onMaximum={(value) => setFilter('stockMax', value)} step="1" /><RangeFilter label="Costo promedio" minimum={filters.costMin} maximum={filters.costMax} onMinimum={(value) => setFilter('costMin', value)} onMaximum={(value) => setFilter('costMax', value)} /><RangeFilter label="Precio de venta" minimum={filters.priceMin} maximum={filters.priceMax} onMinimum={(value) => setFilter('priceMin', value)} onMaximum={(value) => setFilter('priceMax', value)} /></div></section>
...
function RangeFilter({ label, minimum, maximum, onMinimum, onMaximum, step = '0.01' }: { label: string; minimum: string; maximum: string; onMinimum: (value: string) => void; onMaximum: (value: string) => void; step?: string }) { return <fieldset><legend>{label}</legend><input aria-label={`${label} mínima`} type="number" min="0" step={step} placeholder="Mín." value={minimum} onChange={(event) => onMinimum(event.target.value)} /><input aria-label={`${label} máxima`} type="number" min="0" step={step} placeholder="Máx." value={maximum} onChange={(event) => onMaximum(event.target.value)} /></fieldset>; }
