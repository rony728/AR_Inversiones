...
export function TransactionPage({ kind }: { kind: "compra" | "venta" }) {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      load(
        sale ? "/catalogo/clientes" : "/catalogo/proveedores",
        sale ? "clientes" : undefined,
      ),
      load(sale ? "/ventas" : "/compras", sale ? "ventas" : "compras"),
    ]).then(
      ([loadedProducts, loadedPartners, loadedContacts, loadedHistory]) => {
  ...
  const total = useMemo(
    () => lines.reduce((sum, line) => sum + line.cantidad * line.valor, 0),
    [lines],
  );
  const saleSummary = useMemo(
    () => saleTotals(lines, products),
    [lines, products],
  );
  const saleIssue = sale ? validateSaleLines(lines, products) : null;
  const insufficientFunds =
    !sale && exceedsKnownBalance(productsFund?.balance ?? null, total);
  ...
  const saleFundBalance = productsFund
    ? productsFund.balance + saleSummary.subtotal
    : null;
  const change = (index: number, update: Partial<Line>) =>
    setLines((current) =>
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (
      ...
      !custodyId ||
      lines.some(
        (line) => !line.productoId || line.cantidad < 1 || line.valor <= 0,
      )
    ) {
    ...
    if (!sale && duplicateProductId(lines)) {
      setMessage(
        "Cada producto puede aparecer una sola vez dentro de la compra.",
      );
      return;
    }
    if (sale && saleIssue) {
    ...
    const payload = sale
      ? {
          clienteId: contactId || undefined,
          fecha: `${date}T12:00:00.000Z`,
          observaciones: observations.trim() || undefined,
          items: lines.map(({ productoId, cantidad, valor }) => ({
            productoId,
            socioId: partnerId,
            custodiaId: custodyId,
            cantidad,
            precioUnitario: valor,
          })),
        }
      : {
          proveedorId: contactId || undefined,
          socioId: partnerId,
          custodiaId: custodyId,
          fecha: date,
          observaciones: observations.trim() || undefined,
    ...
    try {
      if (!navigator.onLine) {
        ...
        setMessage(
          `${sale ? "Venta" : "Compra"} guardada localmente; se sincronizará al recuperar conexión.`,
        );
        if (sale) {
        ...
        if (sale) {
          setContactId("");
          setObservations("");
          const [updatedProducts, updatedPartners] = await Promise.all([
            load("/catalogo/productos", "productos"),
            load("/catalogo/socios", "custodias"),
          ]);
  ...
  return (
    <>
      <section className="page-title">
        <div>
          <p className="eyebrow">
            {sale ? "VENTA DE CONTADO" : "ENTRADA DE INVENTARIO"}
          </p>
          <h2>{sale ? "Registrar venta" : "Registrar compra"}</h2>
          <p>
            {sale
              ? "El ingreso va al fondo PRODUCTOS seleccionado; el stock siempre es general del negocio."
              : "El socio financia desde su fondo PRODUCTOS; las unidades entran al inventario general."}
          </p>
        </div>
      </section>
      <form className="transaction-form" onSubmit={submit}>
        <section className="panel">
          <div className="form-grid">
            ...
            <label>
              {sale
                ? "Socio responsable del fondo receptor"
                : "Socio responsable del fondo"}
              <select
                value={partnerId}
          ...
          {partnerId && (
            <div
              className={`purchase-fund-summary ${sale ? "sale-fund-summary" : ""} ${insufficientFunds || missingProductsFund ? "insufficient" : ""}`}
            >
              {productsFund ? (
                sale ? (
                  <>
                    <div>
                      <span>Saldo actual · fondo PRODUCTOS</span>
                      <strong>{formatMoney(productsFund.balance)}</strong>
                    </div>
                    <div>
                      <span>Ingreso de la venta</span>
                      <strong>{formatMoney(saleSummary.subtotal)}</strong>
                    </div>
                    <div>
                      <span>Saldo estimado después de confirmar</span>
                      <strong>{formatMoney(saleFundBalance ?? 0)}</strong>
                    </div>
                  </>
                ) : (
                  <>
                    <div>
                      <span>Saldo disponible · fondo PRODUCTOS</span>
                      <strong>{formatMoney(productsFund.balance)}</strong>
                    </div>
                    <div>
                      <span>Saldo después de la compra</span>
                      <strong>{formatMoney(remainingBalance ?? 0)}</strong>
                    </div>
                  </>
                )
              ) : (
                <p>
                  El socio seleccionado no tiene un fondo PRODUCTOS disponible.
                </p>
              )}
            </div>
          )}
        ...
        <section className="panel">
          <div className="panel-head">
            <div>
              <h3>Productos</h3>
              <p>
                Todos los productos pertenecen al inventario general de AR
                Inversiones.
              </p>
            </div>
            <button
          ...
          {lines.map((line, index) => {
            const selectedInOtherLines = lines
              ...
              .map((otherLine) => otherLine.productoId)
              .filter(Boolean);
            const product = products.find(
              (candidate) => candidate.id === line.productoId,
            );
            const estimate = saleLineEstimate(line, product);
            return (
              <div
                className={`transaction-line-group ${sale && estimate.profit < 0 ? "line-loss" : ""}`}
                key={index}
              >
                <div className="line-item">
                  <div className="product-picker-field">
                    ...
                    <ProductSearchSelect
                      ...
                      selectedId={line.productoId}
                      excludedIds={selectedInOtherLines}
                      showStock={sale}
                      onChange={(productoId) => {
                        const selectedProduct = products.find(
                          (candidate) => candidate.id === productoId,
                        );
                        change(index, {
                          productoId,
                          ...(sale
                  ...
                  <label>
                    ...
                    <input
                      ...
                      step="1"
                      max={
                        sale && line.productoId
                          ? availableFor(line.productoId)
                          : undefined
                      }
                      value={line.cantidad}
                      onChange={(event) =>
                        change(index, { cantidad: Number(event.target.value) })
                      }
                    />
                  </label>
                  <label>
                    {sale ? "Precio unitario" : "Costo final por unidad"}
                    <input
                      type="number"
                      min="0.01"
                      step="0.01"
                      value={line.valor || ""}
                      onChange={(event) =>
                        change(index, { valor: Number(event.target.value) })
                      }
                    />
                  </label>
                ...
                {sale && product && (
                  <div className="sale-line-info">
                    <span>
                      Existencia actual
                      <strong>{product.cantidad_disponible}</strong>
                    </span>
                    <span>
                      Existencia posterior
                      <strong>
                        {Number(product.cantidad_disponible) - line.cantidad}
                      </strong>
                    </span>
                    <span>
                      Costo promedio
                      <strong>{formatMoney(product.costo_promedio)}</strong>
                    </span>
                    <span>
                      Precio configurado
                    ...
                    <span>
                      Ganancia estimada
                      <strong>{formatMoney(estimate.profit)}</strong>
                    </span>
                  </div>
                )}
              </div>
        ...
        <section className="panel purchase-observations">
          <label>
            ...
            <textarea
              ...
              placeholder={`Detalles adicionales de la ${sale ? "venta" : "compra"}…`}
            />
          </label>
          <small>{observations.length}/2,000 caracteres</small>
        </section>
        <section
          className={`transaction-total ${sale ? "sale-transaction-total" : ""}`}
        >
          <div className="transaction-main-total">
            <span>Total {sale ? "de la venta" : ""}</span>
            <strong>{formatMoney(sale ? saleSummary.subtotal : total)}</strong>
          </div>
          {sale && (
            <>
              <div>
                <span>Costo del inventario</span>
                <strong>{formatMoney(saleSummary.cost)}</strong>
              </div>
              <div>
                <span>Ganancia estimada</span>
                <strong>{formatMoney(saleSummary.profit)}</strong>
              </div>
            </>
          )}
          <button
            ...
            disabled={
              processing ||
              missingProductsFund ||
              (sale && Boolean(saleIssue))
            }
          >
            {processing
              ? `Registrando ${sale ? "venta" : "compra"}...`
              : navigator.onLine
                ? `Confirmar ${sale ? "venta" : "compra"}`
                : "Guardar sin conexión"}
          </button>
        </section>
        {!sale && insufficientFunds && (
          <p className="purchase-warning">
            El fondo PRODUCTOS quedará con saldo negativo. La compra puede
            confirmarse y quedará respaldada por sus movimientos.
          </p>
        )}
        {sale && saleIssue && (
          <p className="purchase-warning">{saleIssue.message}</p>
        )}
      ...
      <section className="panel list-panel transaction-history">
        <div className="panel-head transaction-history-head">
          <div>
            <h3>{sale ? "Ventas registradas" : "Compras registradas"}</h3>
            <p>
              Consulta rápida de las operaciones disponibles en este
              dispositivo.
            </p>
          </div>
          <strong>
        ...
        <div className="table-tools transaction-history-tools">
          <div className="search">
            <Search size={16} />
            <input
              value={historySearch}
              onChange={(event) => setHistorySearch(event.target.value)}
              placeholder={`Buscar ${sale ? "venta" : "compra"}…`}
            />
            {historySearch && (
              <button
                type="button"
              ...
              {filteredHistory.length ? (
                filteredHistory.map((row) => (
                  <tr key={String(row.id)}>
                    ...
                    {sale && (
                      <td>{formatMoney(row.ganancia_total as string)}</td>
                    )}
                    <td>
                      <strong>{formatMoney(row.total as string)}</strong>
                    </td>
                    <td>
        ...
        <div
          ...
          aria-label={
            sale ? "Ventas en formato móvil" : "Compras en formato móvil"
          }
        >
          {filteredHistory.length ? (
            filteredHistory.map((row) => (
                ...
                <div className="mobile-record-grid">
                  ...
                  {sale && (
                    <span>
                      Utilidad
                      <strong>
                        {formatMoney(row.ganancia_total as string)}
                      </strong>
                    </span>
                  )}
                  <span>
