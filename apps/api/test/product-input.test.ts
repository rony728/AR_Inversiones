...
test('un producto nuevo inicia sin aceptar stock ni costo manual', () => {
  const valid = productCreateInput.parse({ codigo: 'SKU-001', nombre: 'Producto', categoriaId: null, precioVenta: 250, activo: true });
...
test('la edición permite categoría nula y rechaza campos contables', () => {
  assert.deepEqual(productUpdateInput.parse({ categoriaId: null, activo: false }), { categoriaId: null, activo: false });
