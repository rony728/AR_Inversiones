...
export async function transferBetweenCustodies(client: PoolClient, input: z.infer<typeof transferInput>, userId?: string) {
  ...
  if (custodias.rows.length !== 2 || custodias.rows.some((custody) => custody.socio_id !== input.socioId)) throw new AppError(422, 'Los fondos deben pertenecer al socio seleccionado.', 'INVALID_CUSTODY');
