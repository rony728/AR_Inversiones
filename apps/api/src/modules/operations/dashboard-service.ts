...
export async function getDashboard(client: PoolClient, desde: string, hasta: string) {
  ...
  const actividad = await client.query(`SELECT * FROM (
    SELECT v.id,'VENTA'::text AS tipo,v.fecha,v.total AS monto,'Venta confirmada'::text AS descripcion,string_agg(DISTINCT s.nombre,', ') AS socio FROM ventas v JOIN detalle_ventas d ON d.venta_id=v.id JOIN socios s ON s.id=d.socio_id WHERE v.estado='CONFIRMADO' GROUP BY v.id
    UNION ALL SELECT c.id,'COMPRA',c.fecha::timestamp,c.total,'Compra de inventario',s.nombre FROM compras c JOIN socios s ON s.id=c.socio_id WHERE c.estado='CONFIRMADO'
    UNION ALL SELECT p.id,'PRESTAMO',COALESCE(p.fecha_desembolso,p.created_at::date)::timestamp,p.capital_original,'Préstamo desembolsado',s.nombre FROM prestamos p JOIN socios s ON s.id=p.socio_id WHERE p.eliminado_at IS NULL AND p.es_heredado=false AND p.estado<>'ANULADO'
    UNION ALL SELECT pp.id,'PAGO_PRESTAMO',pp.fecha_pago::timestamp,pp.monto_total+pp.monto_extra,concat('Pago de préstamo · interés L ',pp.monto_interes,' · capital L ',pp.monto_capital,' · extra L ',pp.monto_extra),s.nombre FROM pagos_prestamo pp JOIN prestamos p ON p.id=pp.prestamo_id JOIN socios s ON s.id=p.socio_id WHERE p.eliminado_at IS NULL AND pp.revertido_at IS NULL
    UNION ALL SELECT g.id,'GASTO',g.fecha::timestamp,g.monto,g.concepto,s.nombre FROM gastos g JOIN socios s ON s.id=g.socio_id WHERE g.estado='CONFIRMADO'
    UNION ALL SELECT t.id,'TRANSFERENCIA_FONDOS',t.fecha,t.monto,'Transferencia entre fondos',concat(so.nombre,' → ',sd.nombre) FROM transferencias_custodia t JOIN socios so ON so.id=t.socio_id JOIN socios sd ON sd.id=t.socio_destino_id
    UNION ALL SELECT d.id,'DISTRIBUCION_UTILIDAD',d.fecha::timestamp,d.utilidad_total,concat('Distribución de utilidad ',COALESCE(d.rubro::text,'sin clasificar')),NULL::text FROM distribuciones_utilidades d
