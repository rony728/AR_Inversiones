...
type DashboardRow = {
  ...
  capital_prestado_actual: string; prestamos_vencidos: number; fondos_totales: string; valor_inventario: string;
};

export function netPeriodProfit(salesProfit: number, interest: number, expenses: number, otherIncome = 0) {
  return Math.round((salesProfit + interest + otherIncome - expenses) * 100) / 100;
}
...
export async function getDashboard(client: PoolClient, desde: string, hasta: string) {
  const summary = await client.query<DashboardRow>(`SELECT
    ...
    COALESCE((SELECT sum(monto_interes) FROM pagos_prestamo WHERE fecha_pago BETWEEN $1 AND $2),0) AS intereses_cobrados,
  ...
  const actividad = await client.query(`SELECT * FROM (
    ...
    UNION ALL SELECT id,'PRESTAMO',COALESCE(fecha_desembolso,created_at::date)::timestamp,capital_original,'Préstamo desembolsado' FROM prestamos WHERE es_heredado=false
    UNION ALL SELECT id,'PAGO_PRESTAMO',fecha_pago::timestamp,monto_total,concat('Pago de préstamo · interés L ',monto_interes,' · capital L ',monto_capital) FROM pagos_prestamo
  ...
  const ventas = Number(row.ventas_periodo); const gananciaVentas = Number(row.ganancia_ventas); const gastos = Number(row.gastos_periodo); const intereses = Number(row.intereses_cobrados); const otrosIngresos = 0;
  return {
    periodo: { desde, hasta },
    indicadores: { ventas, gananciaNeta: netPeriodProfit(gananciaVentas, intereses, gastos, otrosIngresos), gastos, intereses, capitalPrestadoActual: Number(row.capital_prestado_actual), prestamosVencidos: Number(row.prestamos_vencidos), fondosTotales: Number(row.fondos_totales), valorInventario: Number(row.valor_inventario) },
    resultado: { ventas, gananciaVentas, intereses, otrosIngresos, gastos, gananciaNeta: netPeriodProfit(gananciaVentas, intereses, gastos, otrosIngresos) },
